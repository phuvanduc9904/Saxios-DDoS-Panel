...
// Structure representing a pseudo header needed for TCP header checksum calculation
...
// Function to generate a TCP packet
int generateTCPPacket(char *datagram, struct connection *conn, uint32_t src, uint32_t dst, uint16_t sport, uint16_t dport, uint32_t seq, uint32_t ack, uint8_t flags, char *data, size_t data_len) {
    conn->trip = getCurrentTime() / 1000000;

    int win[] = {64240, 64320, 65535};
    int sel_win = htons(win[rand() % 3]);
    ...
    if (flags & TH_SYN) {
        uint16_t mss[] = { 1480, 1460, 1440, 1420, 1400, 1380 };

        uint8_t scaling[] = { 7, 8, 9 };

        uint16_t sel_mss = htons(mss[rand() % 6]);
...
// Custom strtok function that modifies the input string
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        uint64_t mytime1 = getCurrentTime();
        ...
        if (active > 0) {
            if (getCurrentTime() - delay >= r) {
                delay = getCurrentTime();
                while (1) {
                    struct connection *conn = &conns[conni % cons];

                    if (conn->state == 2) {
                        if (getCurrentTime() - conn->resp >= 1000000000 && data_size < 1333) {
                            conn->state = 0;
                            curr--;
                            active--;
                            break;
                        ...
                        int datagram_len = generateTCPPacket(datagram, conn, conn->saddr, conn->daddr, conn->sport, conn->dport, conn->seq, conn->ack, TH_ACK | TH_PUSH, b, data_size);
        ...
        for (int i = 0; i < cons; i++) {
            ...
            if (conn->state > 1 || (conn->state == 1 && getCurrentTime() - conn->time < 1000000000))
                continue;

            if (getCurrentTime() - newDelay < 80000000)
                continue;

            newDelay = getCurrentTime();
            ...
            conn->tsval = 1241278411 + (rand() % 1241278411);
            conn->tsecr = 0;
            conn->seq = 1247124 + rand() % 127849214;
            conn->ack = 0;

            int datagram_len = generateTCPPacket(datagram, conn, conn->saddr, conn->daddr, conn->sport, conn->dport, conn->seq, conn->ack, TH_SYN, "", 0);

            conn->state = 1;
            conn->time = getCurrentTime();
            bandwidth += datagram_len;
            sendto(raw_fd, datagram, datagram_len, 0, (struct sockaddr *)&conn->addr, sizeof(struct sockaddr_in));
            curr++;
            ...
            if (data_size == 1333) {
                ...
                conn->scaling = 7;
                int datagram_len = generateTCPPacket(datagram, conn, conn->saddr, conn->daddr, conn->sport, conn->dport, conn->seq, conn->ack, TH_ACK, "", 0);
                sendto(raw_fd, datagram, datagram_len, 0, (struct sockaddr *)&conn->addr, sizeof(struct sockaddr_in));
                bandwidth += datagram_len;
                sentt++;
                sends++;
                        ...
                        for (int j = 0; j < cons; j++) {
                            ...
                            if (tcph->source == dest_port && tcph->dest == conn->sport) {
                                ...
                                if (tcph->ack) {
                                    ...
                                    if (tcpdatalen > 0) {
                                        if (getCurrentTime() - conn->rett > 500000000) {
                                            conn->rett = getCurrentTime();
                                            conn->ack += tcpdatalen;
                                            int datagram_len = generateTCPPacket(datagram, conn, conn->saddr, conn->daddr, conn->sport, conn->dport, conn->seq, conn->ack, TH_ACK, "", 0);
                                            sendto(raw_fd, datagram, datagram_len, 0, (struct sockaddr *)&conn->addr, sizeof(struct sockaddr_in));
